...
- function/variable are in general `camelCase`(unexported), `CamelCase`(exported)
...
# create a new module for future release
go mod init github.com/sugoiServal/<prj_name>
```

### Go commands

```bash
# Compile the package (the current directory),
go build && ./<ModuleName>  # run the binary
# (package include `main.go`) build an executable.
# (package not include `main.go`) still compile, useful for checking for compile errors


# compiles and run the file (must include `main.go`)
go run main.go
...
- `* Operator` (dereferencing operator)
  ...
  - used to access the value of an address (`*var`)
- `& operator` (address operator)
  - &var: return a pointer to a variable
    - x := &var implys that x is a pointer
...
// `receiver`: r Rectangle, less common
func (r Rectangle) Area() float64 {
	return r.Width * r.Height
}
...
- ❗`pointer recivers` is more commonly used than value recivers
  - a method with `pointer recivers` can modify attributes in the caller struct
  - a method with `value recivers` cannot
- general guide:
  - `use pointer recivers`
  - if you really don't need to modify the attribute inside call struct, use value reciver
...
c.setColor("blue")
fmt.Println(c.color)  // still white, setColor() is useless
```
