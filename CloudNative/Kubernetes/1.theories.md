...
- `load balancing` performed by `kube-proxy` and the load balancing concept associated with a `Service` the same thing. kube-proxy in the implemenation layer, and , Service Load Balancing in the Abstraction Layer. See [### `Service`: perment IP + Load Balancer]
