...
- Abstract Resources
  ...
  - `Service`: expose service to outside cluster, achieve service discovery & load balancer (layer 4, TCP/UDP).
    ...
    - ExternalName
    - HeadLiness
  - `Ingress`: provides basic HTTP and HTTPS routing to services based on rules, layer 7 (HTTP) load balancer
  ...
  - `Label`: used to group pods (define pods that an config applies to)
