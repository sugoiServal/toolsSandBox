...
### `Pods`: an Application Instance + High Availablility

- pod are the `smallest (basic) deployable element`. It is an `abstration of an application` (could be `multiple containers`).
  ...
  - When a Pod runs `multiple containers`, the containers share the Pod's resources (incl IP address).
- pod are usually `replicated` for high `availability` and DR
...
- Service also responsible to `load balancing`
  - you replicate Pods though `deployment`, or use the "`Horizontal Pod Autoscaling` (HPA)".
  - attach all Pods to a Service, hence them `all belongs to a service`
  - when user hits a service, service balance loads to all healthy pods
...
- `Controller Manager` manage many different `controllers programs` in k8s (think plugins), like
  ...
  - `Replica Controller`: ensure #pod/replicas are available all times
...
- Ensure incoming traffic from either `Ingress or other service or the master node` can reach a `specific destination` in the node, typically pods running on the node.
