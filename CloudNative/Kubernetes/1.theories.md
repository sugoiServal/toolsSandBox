...
- `Load Balancing`: also, when one service hit another service, the requests to theses pod are `load balanced`
- Components
...
- After Creation, the service can be `accessed within cluster` by either
  - `ClusterIP`
  - or the `Service Name (actaully a DNS name)`:
    - <service-name>.<namespace>.svc.cluster.local
    - http://my-clusterip-name.default.svc.cluster.local
